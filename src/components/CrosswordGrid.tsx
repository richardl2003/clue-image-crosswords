...
const CrosswordGrid: React.FC<CrosswordGridProps> = ({
  ...
  const handleDoubleClick = (row: number, col: number) => {
    // First ensure this cell is part of both across and down words
    const hasAcrossWord = grid[row].some((cell, index) => {
      if (index === col) return true;
      if (index < col && !cell.isBlack) return true;
      if (index > col && !cell.isBlack) return true;
      return false;
    });

    const hasDownWord = grid.some((rowData, index) => {
      if (index === row) return true;
      if (index < row && !rowData[col].isBlack) return true;
      if (index > row && !rowData[col].isBlack) return true;
      return false;
    });

    // Only toggle direction if the cell is part of both across and down words
  ...
  const isHighlighted = (row: number, col: number) => {
    ...
    if (activeDirection === "across" && row === activeClue.row) {
      return (
        col >= activeClue.col && col < activeClue.col + activeClue.answer.length
      );
    } else if (activeDirection === "down" && col === activeClue.col) {
      return (
        row >= activeClue.row && row < activeClue.row + activeClue.answer.length
      );
    }
    return false;
  };
